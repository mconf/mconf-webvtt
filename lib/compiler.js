...
/**
 * Compile a custom note in a meta block.
 *
 * @param {meta} meta Object of content for the note
 * @param {output} output String with the meta custom note
 *
 */
function compileMeta (meta) {
  ...
  if (typeof meta.identifier !== 'string' && meta.identifier !== null) {
   throw new CompilerError(`Meta malformed: identifier value is not a string.
   ${JSON.stringify(cue)}`);
  }

   let output = `NOTE ${META} ${JSON.stringify(meta)}\n`;
